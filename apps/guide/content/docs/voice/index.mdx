...
"Voice" refers to Discord bots being able to send audio in voice channels. This is supported in discord.self via [@discord.self/voice](https://github.com/marioparaschiv/discord.self/tree/main/packages/voice), a standalone library made by the developers of discord.self. While you can use it with any Node.js Discord API library, this guide will focus on using it with discord.self.

## Installation

### Barebones

To add voice functionality to your discord.self bot, you will need the `@discord.self/voice` package. If your system does not support aes-256-gcm you also need one of the encryption packages listed below. For example:
...
- [`@discordjs/opus`](https://github.com/marioparaschiv/discord.self) (best performance)
...
The library includes a helper function that helps you to find out which dependencies you've successfully installed. This information is also very helpful if you ever need to submit an issue on the `@discord.self/voice` issue tracker.

```js
const { generateDependencyReport } = require('@discord.self/voice');
...
- **Encryption Libraries**
  - You should have at least one encryption library installed to use `@discord.self/voice`.
- **FFmpeg**
