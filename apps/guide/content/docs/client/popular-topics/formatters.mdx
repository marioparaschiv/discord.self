...
discord.self provides the `formatters` package which contains a variety of utilities you can use when writing your Discord bot.
...
```js
const { blockQuote, bold, italic, quote, spoiler, strikethrough, underline, subtext } = require('discord.self');
const string = 'Hello!';
...
```js
const { hyperlink, hideLinkEmbed } = require('discord.self');
const url = 'https://discord.self.org/';

const link = hyperlink('discord.self', url);
...
```js
const { inlineCode, codeBlock } = require('discord.self');
...
```js
const { time, TimestampStyles } = require('discord.self');
const date = new Date();
...
```js
const { channelMention, roleMention, userMention } = require('discord.self');
