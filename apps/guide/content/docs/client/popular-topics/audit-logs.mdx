...
Let's start by glancing at the `Guild#fetchAuditLogs` method and how to work with it. Like many discord.self methods, it returns a [`Promise`](../additional-info/async-await) containing the `GuildAuditLogs` object. This object has one property, `entries`, which holds a [`Collection`](../additional-info/collections) of `GuildAuditLogsEntry` objects, and consequently, the information you want to retrieve.
...
```js
const { AuditLogEvent } = require('discord.self');
...
```js
const { AuditLogEvent, Events } = require('discord.self');
