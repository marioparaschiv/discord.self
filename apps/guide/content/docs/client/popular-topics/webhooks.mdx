...
Webhooks can send messages to a text channel without having to log in as a bot. They can also fetch, edit, and delete their own messages. There are a variety of methods in discord.self to interact with webhooks. In this section, you will learn how to create, fetch, edit, and use webhooks.
...
<Callout>
	If you would like to read about using webhooks through the API without discord.self, you can read about them
...
### Creating webhooks with discord.self
...
```js
const { EmbedBuilder, WebhookClient } = require('discord.self'); // [!code word:WebhookClient]
...
```js
const { Client, EmbedBuilder, Events, GatewayIntentBits } = require('discord.self');
