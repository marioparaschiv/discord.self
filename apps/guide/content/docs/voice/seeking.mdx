...
There is no built-in method for seeking audio resources in discord.self. However external libraries such as `prism-media` can be used to tackle this issue.
...
```js
const { createAudioResource, createAudioPlayer } = require('@discord.self/voice');
const fs = require('fs');
