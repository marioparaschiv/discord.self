...
discord.self features the `EmbedBuilder` utility class for easy construction and manipulation of embeds.

```js
// at the top of your file
const { EmbedBuilder } = require('discord.self');

// inside a command, event listener, etc.
const exampleEmbed = new EmbedBuilder()
	.setColor(0x0099ff)
	.setTitle('Some title')
	.setURL('https://discord.self.org/')
	.setAuthor({ name: 'Some name', iconURL: 'https://i.imgur.com/AfFp7pu.png', url: 'https://discord.self.org' })
...
```js
const { AttachmentBuilder, EmbedBuilder } = require('discord.self');
// ...
...
```js
const { AttachmentBuilder } = require('discord.self');
// ...
const file = new AttachmentBuilder('../assets/discordjs.png');
