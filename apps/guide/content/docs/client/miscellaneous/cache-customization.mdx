...
Sometimes, you would like to be able to customize discord.self's caching behavior in order to reduce memory usage.
To this end, discord.self provides you with two ways to do so:
...
```js
const { Client, Options } = require('discord.self');
