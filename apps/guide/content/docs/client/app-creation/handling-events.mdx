...
Node.js uses an event-driven architecture, making it possible to execute code when a specific event occurs. The discord.self library takes full advantage of this. You can visit the `Client` documentation to see the full list of events.
...
const { Events } = require('discord.self');
...
const { Events, MessageFlags } = require('discord.self');
...
const { Client, Collection, GatewayIntentBits } = require('discord.self');
...
<Callout>
	In most cases, you can access your `client` instance in other files by obtaining it from one of the other discord.self
