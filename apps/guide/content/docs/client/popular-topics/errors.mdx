...
The code is also handy if you want only to handle a specific error. Say you're trying to delete a message which may or may not be there, and wanted to ignore unknown message errors. This can be done by checking the code, either manually or using discord.self constants.
...
```js
const { RESTJSONErrorCodes } = require('discord.self');
...
## Common discord.self and API errors
...
<Callout>
	**Before** the release of **version 12**, there used to be an issue where the token was not prefixed correctly, which
	...
	sure you have updated discord.self to the current stable version.
</Callout>
