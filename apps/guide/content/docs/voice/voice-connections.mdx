...
```js
const { joinVoiceChannel } = require('@discord.self/voice');
...
```js
const { getVoiceConnection } = require('@discord.self/voice');
...
<Callout>
    **Voice connections can be subscribed to one audio player at most.** If you try to subscribe to another audio player while already subscribed to one, the current audio player is unsubscribed and replaced with the new one.
    ...
    It is also worth noting that the **GuildVoiceStates** [gateway intent](../client/popular-topics/intents) is required. Without it, the connection will permanently stay in the `signalling` state, and you'll be unable to play audio.
...
```js
const { VoiceConnectionStatus } = require('@discord.self/voice');
...
```js
const { VoiceConnectionStatus, entersState } = require('@discord.self/voice');
